...
namespace SleepService.Utils
{
    public class PortControlHelper
    {
        ...
        public Encoding EncodingType { get; set; } = Encoding.BigEndianUnicode;
        ...
        private void DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            {
                string strtem;
                strtem = Convert.ToString(buffer[i], 16).ToUpper();
                if (strtem.Length == 1) { strtem = "0" + strtem; }
                str += strtem;
            }
            //Pb.RcvDev += str;
